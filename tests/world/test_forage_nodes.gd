...
func test_forage_nodes_cover_z01_to_z03_with_rising_mushroom_risk() -> void:
	...
	for node: Node in forage.get_children():
		...
		var zone := valley.zone_at_world(item.global_position)
		assert_true(zone in counts, "%s must sit in the open slice" % item.name)
		counts[zone][item.item_id] = int(counts[zone].get(item.item_id, 0)) + 1
...
func test_scene_owned_forage_nodes_receive_difficulty_respawn_registration() -> void:
	...
	for node: Node in main.get_node("ForageNodes").get_children():
		var item := node as WorldItem
		assert_not_null(item.owner, "%s is scene-owned" % item.name)
		assert_gt(item.resource_respawn_seconds, 0.0, "%s respawns" % item.name)
