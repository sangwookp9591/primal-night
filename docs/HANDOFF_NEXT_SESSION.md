...
> 갱신: 2026-07-19(2차), 코디네이터(Fable 5). 이 문서 하나로 컨텍스트 없이 이어받을 수 있게 쓴다.
...
- **HEAD**: `a3def3c` (main). 작업 트리 클린. **미push 커밋 3건**(`1774dff` Z03 존 판정 버그 수정, `a3def3c` /simplify 보류 2건 이행, 인수인계 문서 갱신) — 임의 push 금지(요청 시에만).
- **테스트**: GUT **681/681 실패 0** (676 + 회귀 테스트 5). 하네스 전부 초록. combat '사체 복제' 플레이크는 조건 대기 전환으로 해소됨(`07c8cb9`, 18연속 PASS).
...
| # | 작업 | 담당 | 비고 |
|---|---|---|---|
| 1 | **사람 플레이테스트 실행** — docs/playtest/PLAYTEST_01_OBSERVATION_SHEET.md | **사용자**+5명 | 최우선 관문. **통과 전 신규 콘텐츠 확장 금지**. 관찰 포인트: Esc/Tab 혼동, 랩터 순찰이 너무 정적인지(배회 3초에 38px), 리허설 발견 4건(§6.5 비고) |
...
| 3 | ~~Z03 오기록 버그~~ **완료** `1774dff` | — | 원인: zone_at_world 정수 나눗셈 0방향 절단(음수 좌표가 청크 0으로 접힘). floori 수정 + 회귀 테스트 |
| 4 | /simplify 보류: ①횃불 시그널 이관 ②SessionClock phase_progress() **완료** `a3def3c`. ③멀티 피해 이펙트 중복만 잔존 | gpt-5.6-sol | ③은 솔로 전환 결정과 함께. 횃불 광원은 밤 GUI 실기로 장착 즉시 점등·해제 즉시 소등 검증됨 |
| 5 | GUI 실기 잔여: 모닥불 점화→굽기·스프, 창 제작→랩터 전투, 올가미 포획 실기 | Fable 5 | **리허설 1회 수행**(신규판 온보딩→줍기→즉석제작→돌칼/횃불 장착·밤 광원). 잔여 흐름은 재료 체인이 김: sinew=사체 해체 선행, 올가미=제작법 발견 필요. 리허설 관찰 4건 — ①첫날 낮이 짧아 모닥불(돌3+나무2) 재료가 빠듯 ②C 즉석제작이 우선순위 자동이라 대상 선택 불가 ③스캐빈저 접근 시 도주(근접 사냥 불가, 올가미 유도 유효) ④굽기·창 도달률은 플레이테스트 핵심 관찰 포인트 |
| 6 | 아트 후속: ~~캐릭터 재작업~~ **완료** `6789262`(시트 14종 3톤 재드로잉+방향 계약 테스트, GUT 682). 잔여: 랩터 사체 어두운 날개/뼈 파트, 보관함 상자 렌더 | gpt-5.6-sol | 캐릭터는 사용자 재판정 대기. 시트 교체 후 `--import` 없이는 갤러리·게임이 구 텍스처 합성함(함정) |
