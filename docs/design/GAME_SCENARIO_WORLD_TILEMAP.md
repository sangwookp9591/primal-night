...
5. **공룡 역할 5종은 유지하되 6개월 MVP의 완전 AI는 최대 3종**: 기존 정본은 출시 공룡 5종을 목표로 한다. 프롬프트의 MVP 행동 유형 상한과 현재 랩터 1종 검증 수준을 고려해 랩터·청소동물·방패뿔까지만 24주 범위로 두고, 돌등·재왕은 정식 버전 확장으로 분리한다.
...
| 중립 존재 | 소형 초식동물, 청소동물, 이동하는 대형 초식 무리 | 위험의 조기 신호이자 자원·경로를 바꾸는 생태 지표다 | 정식 버전 단계 도입 |
| 적대 존재 | 랩터, 둥지 방어 각룡, 후반 최상위 포식자 | 서로 다른 감각과 대응을 요구한다 | 랩터 우선, 나머지는 범위 관문 후 |
...
| 재왕 / 최상위 포식자 | 화산재 속에서 등선만 보이는 대형 수각류 | 후반에 다른 공룡을 사냥하며 계곡을 횡단 | 멀리서 저주파 울음, 작은 공룡 도주, 지면 진동 | 처치 불가. 6~7일 차 경로 재편 장치. **범위 위험**: 완전 AI는 동시 1마리 |
...
| Z05 검은 능선 | 화산 사면·용암동굴·신호대 | 최종 목표, 부품 C, 탈출 | 흑요석, 전자 부속, 조명탄 | 재왕 1마리, 후반 랩터 | 화산재, 낙석, 저온, 7일 화쇄류 | 균열 원인과 엔딩 선택 | 신호기 보정·최종 탈출 |
...
| `bone` 뼈 | 재료 | 0.4 / 10 | — / 보통 | 미끼·도구 | nest, carcass / hard |
...
지정 자리에만 스냅되는 생존 설비는 별도로 다음 6개를 상한으로 둔다.
...
핵심 제작 14개 가운데 모닥불이 이미 포함되므로, 나머지 지정 설비 5개를 모두 추가해도 총 제작법은 19개다. 기존 정본의 20개 상한을 넘지 않는다.
...
프롬프트의 “MVP 행동 유형 최대 3개”는 **AI 역할 3종**으로 제한한다. 이미 검증된 랩터 4상태를 3상태로 억지로 합치지 않는다.
...
| 갈고리턱 청소동물 | 음식 냄새, 바닥 아이템, 큰 소리 | 먹이 탐색·절도·둥지 귀환 | 위협받으면 경계음 후 도주 | 랩터를 부르거나 먹이를 재배치 | 훔친 아이템 ID와 둥지 ETA |
| 방패뿔 초식동물 | 전방 시야, 둥지 거리, 큰 소리 | 물·먹이 이동, 둥지 경계 | 땅 긁기→돌진→복귀 | 랩터 접근 시 방어, 무리 경계음 | Zone 경로·둥지 상태·ETA |

돌등 무리와 재왕은 정식 버전 확장이다. 랩터 핵심 재미가 외부 플레이테스트에서 통과하기 전에 만들지 않는다.
...
| 냄새 | 등록형 `SmellSource`, 출혈 EventBus | 128px 셀, 4Hz, 바람 이류·0.85 감쇠 | 자기 셀 농도와 상승 경사 | 호스트 격자, 4Hz 활성 셀 스냅샷 |
...
| R09 신선한 사체 | Z02~Z04, 랩터 없음 | 새 떼·피 냄새가 먼저 보임 | 고기 채집 / 미끼로 둠 / 우회 | 풍부한 음식과 강한 냄새 | 랩터 요약 개체가 해당 청크로 이동 | C02 |
...
| 오브젝트 | 획득·이동·소멸 아이템, 모닥불, 문, 부품, 시체 | 안정 `object_id`, 수량 불변식 |
...
6. 자동 테스트: 이전 두 schema fixture의 왕복, 강제 종료 후 backup 복원, 아이템 총량, 핵심 부품 유일성을 검증한다.
...
| `ActorDefinition` | `id, scene, role, senses, movement, spawn_tags, summary_rules` | 공룡·플레이어 외형/행동 데이터 |
...
| AI 유형 | 랩터 1종 | 최대 3종 | 역할 5종 상한 |
| 완전 AI | 랩터 1마리 기준 | 최대 24, 추격 12 | 동일 상한, 요약 AI 100 |
| 아이템 | 5개 | 30개 상한 | 40개 이하 |
| 제작법 | 모닥불·치료 경로 | 핵심 14개+지정 설비, 총 19개 이하 | 20개 상한 |
| 이벤트 | 감각 루프 고정 하네스 | 고정 10+조건 15+랜덤 20 | 수치 유지, 변형 actions 확대 |
| 엔딩 | 성공/실패 회색 상자 | 기반 3종+표현 6종 | 6종 유지 |
...
- 공간 5개 추가 변형과 기록을 든 귀환/균열 봉합 표현.
- 컨트롤러 전체 완주와 한국어·영어 동시 문자열.

### 22.4 정식 버전 또는 출시 후로 연기

- 돌등 무리·재왕의 완전 행동, 3~4인 협동, 장기 생존 모드.
- 여러 층, 대형 월드, 차량, 자유 건축, 농사, NPC 외교, 전용 서버, 호스트 이전.
- 공룡 길들이기·탑승·교배, PvP, Workshop, 공식 모드 도구.

**범위 위험:** Steam 2인 실기기 통합, 5 Zone 전체 콘텐츠, 저장 migration이 18주 말까지 완주되지 않으면 AI 3종을 랩터+청소동물 2종으로 줄이고 부위 부상·빛 일반화의 시각 변형을 연기한다. 기존 랩터 감각 루프와 탈출 완주는 줄이지 않는다.
...
2주 단위 12 Sprint다. W1~4는 현재 저장소의 검증 결과를 반영해 완료로 표시한다.
...
| W7~8 | 타일·공간 수직 조각 | Z01~Z03 연결 3청크, 레이어 계약, Space/Entrance | 추락→강변→밀림 10~15분 루프 | 타일 계약, Y-Sort, 음소거 위험 신호 | 가림·좌표·nav seam | 50ms 청크 멈춤 0, 길찾기 seam 0 |
| W9~10 | 자원·제작·빛 최소 | 아이템 20개 우선, 제작 10개, 횃불 LightSignature | 냄새 물자·불·미끼로 서로 다른 3해법 | 총량 불변, 권한 변조, Light/AI | 데이터 필드·UI 불일치 | 설명 수치=Resource, B06 예산 통과 |
| W11~12 | 청크·월드 델타·저장 | 40청크 골격, streamer, route/tile/object delta, backup | 왕복 10회, 저장 20회, 강제 종료 복구 | B09/B10, schema fixture | nav 갱신·파일 증가 | 100ms 멈춤 0, 최신/backup 복구 |
| W13~14 | 3일 완주형 콘텐츠 | Z01~Z03, 공간 6개, 이벤트 절반, 부품 A | 외부 설명 없이 3일 목표 완주 | 싱글/2인 완주, 5시드 | 자원 노동·길 찾기 | 5팀 중 4팀 첫 15분 물·불 확보 |
| W15~16 | 5 Zone·부품 루프 | Z04/Z05, 공간 10개, 아이템 30·제작 14 | 부품 3개 회수와 능선 도달 가능 | 시드 10개 도달성·막힘 검사 | 콘텐츠 제작량 | 막힘 0, 평균 60분 내 능선 도달 |
| W17~18 | 7일·AI·이벤트·엔딩 | AI 최대 3종, 이벤트 45, 엔딩 표현 6 | 싱글/2인 전원·부분·실패 완주 | B04/B12, 이벤트 충돌, save/load | AI·이벤트 상태 폭증 | 전체 루프 완주, 신규 기능 동결 |
| W19~20 | 아트·음향·접근성 | 8방향 핵심 시트, 타일, 방향 자막, 한/영 UI | 회색 상자 핵심 기능을 실제 에셋으로 교체 | 음소거·색각·컨트롤러 완주 | 에셋 정합성·번역 길이 | 위험 신호 5/5 인지, 임시 에셋 목록 0 |
| W21~22 | 외부 테스트·성능·네트워크 | 20팀 빌드, 밸런스, 최소사양 기준선 | 진행 차단 동기화·저장 손상 0 | B01~B12, 30분 soak, 250ms/5% | 실제 하드웨어 병목 | 차단 기준 전부 통과 |
| W23~24 | Steam Demo·출시 후보 | depot, 상점과 기능 일치, 복구 로그, 심사 여유 | 클린 Windows 2대 설치→탈출 | 릴리스 빌드 전체 완주 | 심사 수정·막판 회귀 | 체크리스트가 아니라 실제 결과 파일 존재 |
...
| 다수 개체 AI | 중 / 높음 | B04 24완전/12추격 | 틱 분산, 공간 해시, 먼 요약 | AI 3종→2종, 상한 24 유지 |
...
| 파괴 월드 저장 | 중 / 높음 | 1,000 tile delta 왕복 | 베이스+델타, 안정 ID, chunk dirty | 자유 파괴 제거, 지정 상태 교체만 |
| 이벤트 상태 충돌 | 높음 / 높음 | 45이벤트 모든 우선순위 조합·고정 시드 | required/blocked flag, 단일 경로 mutation/틱 | 랜덤 20→10 활성 회전, 데이터는 유지 가능 |
| 저장 파일 증가 | 중 / 높음 | B10 20회, 2MB 재접속·세션 파일 추세 | dirty chunk, 만료 흔적 정리, schema version | 장식 흔적 영구 저장 제거 |
| 콘텐츠 제작량 | 높음 / 높음 | W13 3일 외부 완주 | 공간 10·아이템30·제작14 상한, 변형 프리셋 | 공간 10 고정, AI·부위부상 축소 |
...
| 아트·애니메이션 | 높음 / 중 | 플레이어·랩터 한 세트 실제 회전 | 8방향 일괄 생성, 픽셀 스냅, 팔레트 | 공룡 종류·동작 수 축소, 방향 수는 축소 안 함 |
| 냄새 격자 확장 | 중 / 높음 | B06 원천 증가, 활성 셀·p95 | 등록 목록, 활성 셀만 4Hz, 분산 계산 | 냄새 원천 종류 축소, 핵심 blood/raw_meat/bait 유지 |
...
- 새 공룡, 45개 이벤트 전체, 30개 아이템 전체, 완성 에셋.
