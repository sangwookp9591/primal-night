...
- **F-A 저장 폴백:** W14에 저장·마이그레이션 관문을 통과하지 못하면 장식 타일 델타를 버리고 플레이어·핵심 부품·경로·지정 오브젝트만 저장한다. 원자적 교체, 직전 backup, 순차 migration은 줄이지 않는다.
- **F-B 콘텐츠 폴백:** W18에 5 Zone 골격은 유지하되 대표 공간의 추가 변형을 연기하고, AI는 랩터+청소동물 2종으로 제한한다. 10개 대표 공간의 진입·탈출 경로와 부품 3개 도달성은 유지한다.
- **F-C 이벤트·엔딩 폴백:** 이벤트 데이터 풀 45개는 유지하되 세션별 조건·랜덤 활성 회전을 10~15개로 제한하고, 엔딩은 기반 판정 3종을 먼저 완성한다. 표현 6종과 방패뿔 AI는 관문 통과 후에만 추가한다.
- **F-D 에셋 폴백:** 공룡 종류·동작 수와 공간 장식 변형을 줄이고 임시 에셋 교체 우선순위를 핵심 판독 요소로 한정한다. 위험 방향 8방향, 자막, 색 외 형태 신호는 줄이지 않는다.
...
| W13~14 **범위 위험 F-A** | 저장·backup·migration 전용 | 원자적 저장, 직전 backup, schema fixture, `N→N+1` migration | 저장 20회, 강제 종료 복구, 이전 2개 schema 왕복 | B10, 손상 파일, 핵심 부품 유일성 | 파일 증가·부분 기록 | 최신/backup 복구와 migration 전부 통과; 실패 시 F-A 적용 |
| W15~16 **범위 위험 F-B** | 3일 완주형 콘텐츠 | Z01~Z03, 공간 6개, 아이템 30·제작 14, 부품 A | 외부 설명 없이 싱글/2인 3일 목표 완주 | 5시드 완주, 자원 총량, 길 찾기 | 자원 노동·콘텐츠 제작량 | 5팀 중 4팀이 첫 15분에 물·불 확보; 초과 시 공간 변형부터 축소 |
| W17~18 **범위 위험 F-B** | 5 Zone·7일 부품 루프 | Z04/Z05, 공간 골격 10개, 부품 3개, 기반 결과 판정 3종 | 싱글/2인 부품 회수→능선→전원·부분·실패 완주 | 시드 10개 도달성·막힘·save/load | 70분 콘텐츠 밀도 | 막힘 0, 평균 60분 내 능선 도달; 실패 시 F-B 적용 |
| W19~20 **범위 위험 F-C** | 이벤트 회전·랩터 중심 통합 | EventDirector, 고정 10개, 세션별 조건·랜덤 활성 10~15개, 기반 엔딩 3종 | 이벤트 충돌 없이 세 기반 결과를 각 3회 재현 | B04/B12, 우선순위 조합, 고정 시드 | 이벤트 상태 조합 폭증 | 전체 루프 완주 후 기능 동결; 표현 6종·추가 AI는 F-C 관문 뒤만 허용 |
| W21~22 **범위 위험 F-D** | 핵심 에셋·접근성·외부 테스트 | 8방향 핵심 시트, 타일, 방향 자막, 한/영 UI, 20팀 빌드 | 위험 신호 5/5 인지, 진행 차단 동기화·저장 손상 0 | 음소거·색각·컨트롤러, B01~B12, 30분 soak | 에셋 정합성·실기기 병목 | 차단 기준 전부 통과; 실패 시 F-D 적용 |
...
| 다수 개체 AI | 중 / 높음 | B04 24완전/12추격 | 틱 분산, 공간 해시, 먼 요약 | F-B: AI 3종→2종, 상한 24 유지 |
...
| 콘텐츠 제작량 | 높음 / 높음 | W15 3일 외부 완주 | 공간 10·아이템30·제작14 상한, 변형 프리셋 | F-B: 공간 골격 10 유지, 변형·AI·부위부상 축소 |
...
| 아트·애니메이션 | 높음 / 중 | 플레이어·랩터 한 세트 실제 회전 | 8방향 일괄 생성, 픽셀 스냅, 팔레트 | F-D: 공룡 종류·동작 수 축소, 방향 수는 축소 안 함 |
