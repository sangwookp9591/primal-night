...
| 인벤토리·퀵 제작 | 16칸 슬롯+무게 제한 인벤토리, 날고기 1→미끼 1 `craft_bait`, `NetCrafting` host 권위 복제 | 현재 제작은 W6 한 경로다. 핵심 18개 제작법과 설비 조건은 후속 데이터 확장으로 둔다 |
| 네트워크 | ENet 로컬 어댑터, 호스트 권한 이동·아이템·피해·치료·모닥불·감각·재접속, 30초 아바타 잔류·120초 슬롯, 10Hz 스냅샷 | 싱글도 로컬 호스트 흐름을 사용한다. 3~4인은 별도 부하·동시성 관문 후 연다 |
| 세션 | 3일 압축 `SessionClock`과 위험 노출 후 도착하는 `LoopObjective` | 정식 7일 세션의 검증된 최소 골격으로 재사용한다 |
...
| 항목 | 최종안 |
|---|---|
| 핵심 로그라인 | 시간 균열에 추락한 현대 조난자들이 7일 동안 원시 재료와 남은 현대 잔해로 생존 기술을 재발명하고, 소리·냄새·빛을 관리하며 구조 신호기를 완성해 랩터의 계곡을 탈출한다. |
...
| 제작법 | 모닥불·치료 경로+`craft_bait` 퀵 제작 | 핵심 18개+지정 설비, 고유 총 23개 | 26개 계획, 절대 28개 상한 |
...
| 시간 | 3일 압축 시계 | 7일×10분, 70~90분 세션 | 동일, 선택 장기 모드는 출시 후 |
