...
| 기술 수준 | 소진되는 현대 장비 → 돌·뼈·가죽·수지 기반 원시 도구 → 제한된 현대 잔해를 결합한 생존 공학 |
| 생존의 의미 | **현재 정본:** 출혈·냄새·불 유지·추출 도달·생존 여부 같은 세계 변수로 `STABLE_ESCAPE/FORCED_ESCAPE/REMAIN/FAILED`가 갈리는 것. 7일 차 신호기 부품 탈출은 수직 절편 검증 뒤 재평가할 후속 표현/장기 구조다. |
| 핵심 판타지 | 현대 지식으로 원시 재료의 성질을 배우고, 남은 현대 부품과 결합해 포식자의 결정을 역이용하는 생존 공학자 |

흔한 해석은 “공룡을 쓰러뜨려 장비를 강화하는 액션 생존”이다. 본 설계의 독창적인 해석은 **현대 장비가 소진되는 동안 플레이어가 원시 재료의 성질을 관찰해 기술을 재발명하고, 자신이 만든 소리·냄새·빛의 흔적으로 포식자의 결정을 바꾸는 생존 스릴러**다. 적의 체력보다 `어디에서 무엇을 남겼고, 어떤 재료로 그 흔적을 만들었는가`가 더 중요하다.

한 문장 콘셉트는 다음을 정본으로 한다.

> **현대에서 원시시대로 떨어진 평범한 사람들이 공룡의 감각을 배우고, 돌과 뼈와 남은 현대 기술을 결합해 살아남는 협동 생존 게임.**

### 1.2 입력되지 않은 값에 대한 가정

| 입력 | 가정 | 이유 |
|---|---|---|
| 추가 키워드 | 시간 균열, 화산 계곡, 3구역 수직 절편, 제한된 현대 장비, 환경 서사 | 현행 코드와 새 원칙은 20~30분짜리 Z01~Z03 절편 검증을 우선한다. 7일 탈출은 확장 목표다 |
...
| 목표 플레이 시간 | 현재 검증은 3일 압축 시계와 20~30분 수직 절편. 70~90분 7일 세션은 확장 목표 | 현재 `SessionClock`은 3일×10분이며, 7일 장기 루프는 아직 정본 구현이 아니다 |
| 출시 협동 | 싱글 및 검증된 2인 우선. 코드는 host+3을 수용하지만 4인은 장시간 관문 전 약속하지 않는다 | `NetConfig.max_clients = 3`과 4인 하네스는 수용력/검증 후보이며 출시 사양 문구가 아니다 |
...
| 부상·생존 | 체력, 출혈 지속 피해, 피 냄새, 붕대 지혈, 스태미나, 체온·수분·포만·피로·젖음 단계, 다리 열상 이동 효과 | 수분·포만은 즉사 게이트가 아니라 회복/재생 계열의 압박으로 유지한다 |
| 모닥불 | 지정 자리 설치, 돌 3+나무 2, 1초 설치, 반경 220px, 연료 60초, 랩터 회피, 호스트 권한 | 자유 건축으로 바꾸지 않는다. 장기 세션용 연료 보충만 변경 제안한다 |
| 인벤토리·제작 | 16칸 슬롯+무게 제한 인벤토리, 자동 등록 `ItemData`/`RecipeData`, 원시 제작 다수, `NetCrafting` host 권위 복제 | 새 제작법은 데이터로 확장하되 시간·취소·애니메이션은 별도 구현 전까지 계획으로 표기한다 |
| 장비·전투 | `EquipmentComponent`의 `outfit/back/main_hand`, 장비 스냅샷 복제, `PlayerVisualRig` 레이어, 칼·창 근접과 활/화살 권위 전투 | 구현된 장비/전투는 현재 사실로 쓰되 총기·특수 화살·대형 포식자 처치는 계획/제외로 유지한다 |
| 난이도·진입 | `TitleScreen`의 싱글/호스트/참가, 난이도 3종, 컨트롤 설정과 `DifficultyRuntime` 프리셋 적용 | 난이도는 적 체력 배율이 아니라 자원·흔적·사망 복구·포식자 관용도 축이다 |
| 네트워크 | ENet 로컬 어댑터, 호스트 권한 이동·아이템·피해·치료·모닥불·감각·장비·전투·재접속, 30초 아바타 잔류·120초 슬롯, 10Hz 스냅샷 | 4인은 수용력과 하네스가 있어도 장시간 soak 전 외부 약속으로 쓰지 않는다 |
| 세션 | 3일 압축 `SessionClock`과 위험 노출 후 도착하는 `LoopObjective` 4결과 | 7일 부품 신호기 구조는 현재 정본이 아니라 후속 표현/확장 목표다 |
| 저장 | `SaveService`가 JSON 파일로 난이도·플레이어·시계·목표·월드 아이템·거점·사체·개체·기록을 왕복한다 | 원자적 tmp/backup, 순차 migration, 청크 델타 저장은 아직 계획이다 |
...
2. **3일 압축 시계에서 7일×10분 세션으로 확장(미구현/계획)**: 기존 `SessionClock`의 호스트 권한·스냅샷 계약을 유지하고 날짜·낮/황혼/밤 구간을 데이터화한다.
3. **청크 스트리밍과 월드 델타 저장 추가**: 현재 테스트 월드는 한 장면이다. 타일맵 전체 저장 대신 시드+델타를 사용한다.
4. **2~4인 확장은 출시 1~2인과 분리**: 현재 `NetConfig.max_clients = 3`과 host+3 하네스가 있으나, 장시간 부하·동시성 테스트를 통과한 뒤에만 4인을 노출한다. 수용력과 제품 약속을 구분한다.
...
6. **제한 권총 3발은 6개월 MVP에서 연기**: 현재 전투 구현은 칼·창·활·화살 중심이다. 조명탄·미끼가 “비싸고 큰 감각 사건”을 더 적은 범위로 검증하므로, 총기는 핵심 루프와 저장이 완주된 뒤 정식 버전 후보로만 재평가한다.
...
- 한 문장 소개: 시간 균열 계곡에서 감각 흔적을 관리해 위험 노출을 통제하고 탈출 또는 잔류 결과를 만든다. 7일 신호기 부품 루프는 미구현 확장 후보다.
...
- 단기/장기 목표: 물·불 확보 / Z01~Z03 수직 절편에서 위험 관리와 추출 판단 증명. 부품 3개 회수 후 능선 신호기 가동은 미구현 확장 후보.
- 탐색 이유: 현재는 위험 관리 자원과 지름길을 찾는다. 후속 7일 구조에서는 부품을 서로 다른 위험 규칙의 세 지역에 배치할 수 있다.
...
| 항목 | 최종안 |
|---|---|
| 핵심 로그라인 | 시간 균열에 추락한 현대 조난자들이 원시 재료와 남은 현대 잔해로 생존 기술을 재발명하고, 소리·냄새·빛을 관리하며 랩터의 계곡에서 살아 나갈 조건을 만든다. |
| 플레이어 판타지 | 보이지 않는 포식자의 결정을 읽고, 돌·뼈·가죽·수지와 현대 부품을 미끼·함정·불·탈출 장비로 바꾸는 원시 공학자 |
...
| 행동 이유 | **현재 정본:** 위험 노출을 관리한 채 추출 지점에 도달하거나, 생존 상태로 순환에 남는 판단을 한다. 7일 뒤 능선 신호기 가동은 후속 장기 구조다. |

### 5.2 세계관의 핵심 규칙

> **변경 공지 — 2026-07-18 정렬:** 아래 절의 "핵심 부품 3개 없이는 탈출할 수 없다"는 절대 규칙은 폐기한다. 최신 기준은 고정 부품 납품이 아니라 출혈·냄새·불·추출 도달·생존 여부 같은 세계 변수로 `STABLE_ESCAPE/FORCED_ESCAPE/REMAIN/FAILED`가 갈리는 탈출 구조다. 세부 대조표는 `docs/technical/DESIGN_DOC_CODE_ALIGNMENT.md`를 따른다.
...
8. 현대 장비는 수리·탄약·연료 한계로 소진된다. 영구 상위 티어가 아니라 원시 도구를 만들 시간을 벌고, 후반 혼합 장비의 희귀 부품이 된다.
9. 제작 지식은 레벨이나 설계도 팝업이 아니라 재료 관찰과 실제 실험으로 발견한다. 성공과 실패 결과는 주인공의 노트에 그림과 짧은 관찰문으로 남는다.

게임 내에서 절대로 변하지 않는 규칙은 “감지에는 원인이 있고, 치명적 위협에는 사전 신호가 있으며, 생존 결과는 플레이어가 남긴 위험 흔적으로 설명되어야 한다”다. 핵심 부품 3개와 신호기 안정화 선택은 현재 구현된 탈출 게이트가 아니라, 수직 절편이 안정된 뒤 `STABLE_ESCAPE/FORCED_ESCAPE/REMAIN/FAILED` 위에 얹을 후속 표현 후보로 남긴다.

### 5.3 목표 구조

> **변경 공지 — 2026-07-18 정렬:** 아래 목표 구조의 신호기 부품 A/B/C 회수, 7일 장기 목표, 90초 충전은 현재 수직 절편 정본이 아니다. 최신 진행 목표는 20~30분짜리 3구역 절편에서 세계 변수 기반 탈출 판정을 증명하는 것이다.

| 기간 | 목표 | 실패 시 의미 |
|---|---|---|
| 현재 수직 절편: 20~30분 | Z01~Z03에서 물·불·붕대·기본 제작·위험 노출 관리·추출 지점 도달을 검증 | 도달해도 위험이 남으면 `FORCED_ESCAPE`, 안정 조건을 갖추면 `STABLE_ESCAPE`, 만료 생존은 `REMAIN`, 전멸은 `FAILED` |
| 후속 확장: 3일 완주형 | Z01~Z03 대표 공간, 사체 해체, 건조·뼈 제작, 거점 설비를 포함한 짧은 완주 | 자원 손실·우회로 폐쇄·부상으로 다음 날 계획이 바뀐다 |
| 장기 계획: 7일 신호기 구조 | Z04/Z05, 부품 A/B/C, 능선 신호기, 표현 엔딩을 추가 | 현재 구현 완료가 아니며 관문 통과 뒤에만 정본 목표로 승격한다 |

기존 생존 게임과 다른 점은 허기 수치를 오래 채우는 노동보다 **감각 흔적의 생성·전파·오독이 사건을 만든다는 것**, 그리고 무한 정착이 아니라 매 세션 위험과 철수 판단이 명확한 결과로 남는다는 것이다.
...
> **변경 공지 — 2026-07-18 정렬:** 아래 6개 엔딩 표는 현재 정본 판정이 아니라 후속 표현 후보로만 남긴다. 최신 코드 판정은 `STABLE_ESCAPE/FORCED_ESCAPE/REMAIN/FAILED` 4결과이며, 부품 3개 납품을 필수 조건으로 하지 않는다.

현재 구현된 `LoopObjective`의 판정은 `STABLE_ESCAPE / FORCED_ESCAPE / REMAIN / FAILED` 4종이다. 아래 6개는 이 네 결과 위에 부품·기록·보정·희생 플래그를 조합하는 **미구현/계획 표현 후보**다. 별도 거대 컷신이 아니라 결과 화면·월드 플래그 차이로 제작한다.
...
| 10분 | 현재 한 게임 내 하루: 거점 출발→공간 하나 탐색→황혼 전 귀환 또는 야간 강행 | 물·포만·불 연료·붕대·현대 장비 | 도구 재료/영구 지름길/위험 관리 단서 | 밤, 비, 부상, 귀환로에 형성된 냄새 띠 | 날씨, 길막, 자원, 사건 조합 | 시간, 월드 델타, 제작, 이벤트 |
| 20~30분 | 현재 수직 절편: 3일 압축 안에 Z01~Z03 판단 루프 완주 | 대부분의 시작 현대 장비, 거점 자원, 경로 선택 기회 | `STABLE_ESCAPE/FORCED_ESCAPE/REMAIN/FAILED` 결과와 원인 문장 | 누적 자원 고갈·랩터 조사·부상·동료 사망 | 시드, 시작 위치, 위험 노출 경로 | 세이브, 난이도, 엔딩, 협동 |
| 70~90분 | **미구현/계획:** 7일 신호기 장기 루프 | 부품·보정 재료·능선 진입 비용 | 표현 엔딩 단서와 장기 숙련 지식 | 화산재·무리 이동·신호기 충전 위험 | 부품 위치 변형, 사건 연쇄 | 장기 세이브, 표현 엔딩 |
...
| 확보 | 채집·회수·분해·해체 | 소음·노출 시간 | 자원·기록·위험 관리 선택지 | 랩터 조사 또는 경로 차단 |
| 위기 대응 | 미끼·불·우회·자원 투기·구조 | 소모품·경로·체력 | 생존과 시간 확보 | 아이템/시체/불이 월드에 남음 |
| 귀환·정비 | 지혈·건조·제작·장비 교체 | 거점 자원·낮 시간 | 다음 밤 안전도 | 연료 고갈·거점 냄새 축적 |

### 6.4 보상 주기

- 0~3분: 눈에 보이는 자원 1개 또는 바람·흔적 튜토리얼 정보.
- 3~5분: 작은 공간 하나, 안전한 우회로, 제작 재료 묶음 가운데 하나.
- 5~10분: 위험 관리 진전, 영구 지름길, 희귀 사건, 배경 전용 정보 가운데 하나.
...
| Z04 둥지 평원 | 초지·둥지 고리·마른 수로 | **미구현/계획:** 고위험·고보상, 부품 B 후보 | 뼈, 질긴 가죽, 전자 포드 | 방패뿔, 랩터 사냥조, 무리 이동 | 엄폐 부족, 4일 차 대이동 | 서로 다른 시대 생물이 모인 증거 | 희귀 재료, 무리가 연 새 길 |
| Z05 검은 능선 | 화산 사면·용암동굴·신호대 | **미구현/계획:** 최종 목표, 부품 C, 탈출 표현 후보 | 흑요석, 전자 부속, 조명탄 | 재왕 통로 사건 1회, 후반 랩터 | 화산재, 낙석, 저온, 7일 화쇄류 | 균열 원인과 엔딩 선택 | 신호기 보정·최종 탈출 |
...
경로 화살표는 공간 연결만 뜻한다. 런타임 통행 가능 여부는 `WorldDelta`의 `route_state`와 날짜·날씨 플래그가 결정한다. 현재 수직 절편에서는 40청크 골격을 보존하되 Z01/Z02/Z03만 열린 동선으로 검증하고, Z04/Z05 연결은 자연 경계로 잠긴다.
...
> **변경 공지 — 2026-07-18 정렬:** 아래 표의 5 Zone·40 Chunk MVP 전제는 즉시 콘텐츠 제작 목표가 아니다. 최신 기준은 40청크 골격을 보존하되 `Z01/Z02/Z03` 3구역 수직 절편의 밀도와 판단 주기를 먼저 검증하는 것이다.
...
|---|---|---:|---:|
| World | 시드, 3일 압축 시간, 날씨 순서, 참가자, 4결과 | 1세션 월드 | 7일 시간·표현 엔딩은 확장 목표 |
| Region | 전역 환경 단계와 Zone 연결 | 1개 균열 계곡 | 1개 유지. 대형 월드 증설 안 함 |
| Zone | 생태·자원·스토리 규칙 | 5개 | 5개, 각 Zone 희귀 변형 증가 |
| Chunk | 로딩·오브젝트 활성·델타 저장 | 40개 골격 중 Z01~Z03 절편 우선 / 8×8 경계 | 5 Zone 확장 후 최대 64개 플레이 가능 / 12×8 경계 |
...
> **변경 공지 — 2026-07-18 정렬:** 4인은 최종 목표이지만 `host + 3 clients` 장시간 검증 전에는 출시 사양이나 외부 약속으로 쓰지 않는다. 현재 코드가 4인 수용 하네스를 일부 갖췄더라도 제품 약속은 별도 관문 뒤에만 가능하다.

저장·감각·AI 데이터는 `player_id` 배열과 군집 집합을 사용해 4명을 막지 않는다. 그러나 출시 약속은 검증된 2인 우선이며, 4인은 장시간 관문 뒤에만 외부 사양으로 쓴다.

- 현재 `NetMovement`와 `NetSurvival`의 스냅샷 엔트리 상한 8은 4인을 수용한다.
- 현재 `NetConfig.max_clients = 3`은 host+3을 수용한다.
- 장비 전용 host+3 하네스와 4인 soak 하네스가 있으나, 이 결과는 출시 약속이 아니라 관문 후보다.
...
| S10 검은 능선 신호대 | **미구현/계획:** Z05, 부품 3개 필요 후보 | 부품 C, 최종 신호기, 엔딩 단서 | 이전 가동 실패와 희생 흔적 | 보정 회로실; 7일 차 이중 마감 적용 |
...
| 시간 제한 | 세션 방향과 밤 긴장 | 날짜·구간 | 날씨·사건·엔딩 | 7일 이후 무한 연장 없음은 계획 | **3일 압축 시계 현재, 7일 변경 제안** |
...
현재 `GameData`는 `data/items/*.tres`와 `data/recipes/*.tres`를 디렉터리 스캔해 `id`로 자동 등록한다. `ItemData`는 `id, display_name, stackable, max_stack, weight, smell_*`를 제공하고, 장비는 `WearableData`가 `equip_slot, visual_id, modifiers, noise/smell/wetness, drying_speed, wet_weight_penalty`를 제공한다. 아래처럼 내구·희귀도·태그를 일반화하는 필드는 **미구현/계획**이다.
...
기존 필드를 버리지 않고 `ItemData`/`WearableData`에 필요한 필드만 추가한다. 데이터가 없는 아이템은 로드 시 개발 빌드에서 실패한다. 수동 `ITEM_PATHS` 배열은 쓰지 않으며, 저장 ID는 파일 경로가 아니라 `id`다.

### 14.2 MVP 아이템 34개

`현재`는 이미 데이터 리소스가 존재하는 항목을 가리킨다. 같은 행의 내구·희귀도·태그·확장 사용 효과는 해당 리소스에 필드가 없으면 **미구현/계획**이다.
...
| `fiber` 섬유 **현재** | 원시 재료 | 0.1 / 20 | — / 흔함 | 묶기·붕대 | river, forest / cordage |
...
| `dried_meat` 말린고기 **현재** | 음식 | 0.5 / 5 | 보존 기간은 미구현 / 보통 | 포만 회복, 냄새 10 후보 | rack / food, preserved |
| `bone` 공룡 뼈 **현재** | 사체 재료 | 0.4 / 10 | — / 보통 | 미끼·뼈 제작 | nest, carcass / hard, bone |
| `hide` 가죽 **현재** | 사체 재료 | 1.2 / 5 | 신선/건조는 미구현 / 보통 | 냄새 원천, 방한 장비 재료 후보 | carcass / hide, armor |
| `sinew` 힘줄 **현재** | 사체 재료 | 0.2 / 10 | 신선/건조는 미구현 / 보통 | 결속·덫 후보, 냄새 원천 | carcass / cordage, bone |
...
| `torch` 횃불 **현재** | 빛·손 장비 | 1.0 / 1 | 연소 시간은 미구현 / 보통 | `main_hand` 장비와 시각 레이어. 일반 `LightSignature`는 계획 | craft / light, fuel |
| `stone_knife` 돌칼 **현재** | 도구·무기 | 0.8 / 1 | 내구 미구현 / 흔함 | 근접 거리·전방 호·스태미나·쿨다운 전투 | craft / blade, tool |
| `bone_scraper` 뼈 긁개 **현재** | 해체 도구 | 리소스 기준 / 1 | 내구 미구현 / 보통 | 사체 해체 시간 단축 도구 | craft / bone, butcher, tool |
| `stone_spear` 돌창 **현재** | 도구·방어 | 1.8 / 1 | 내구 미구현 / 보통 | 근접 거리·전방 호·스태미나·쿨다운 전투 | craft / weapon, reach |
| `bow` 활 **현재** | 도구·사냥 | 1.2 / 1 | 내구 미구현 / 보통 | 조준·발사·화살 소모·투사체 회수 | craft / weapon, ranged |
| `small_pack` 작은 가방 **현재** | 등 장비 | 1.4 / 1 | 내구 미구현 / 보통 | `back` 슬롯, `placeholder_back` 시각. 용량 증가는 modifier 수치만 있고 시스템 효과는 미구현 | craft / carry, back |
| `hide_mantle` 가죽 망토 **미구현/계획** | 몸 장비 | 1.5 / 1 | 5일 / 보통 | 추위 감소 15%, 젖으면 무게·이동 소음 증가 | craft / hide, armor |
...
> **각주 — `현재` 태그 범위:** 해당 아이템의 `.tres` 데이터 리소스가 현재 저장소에 존재한다는 뜻으로 사용한다. 장비·전투·해체처럼 코드가 실제로 읽는 필드는 현재 구현으로 적고, 내구·희귀도·태그·부패·용량 확장처럼 코드 경로가 없는 효과는 **미구현/계획**으로 본다.

`hide`, `sinew`, `bone_scraper`, `small_pack`은 현재 데이터가 존재한다. `hide_mantle`, `dinosaur_tooth`, `tooth_knife` 같은 후속 항목은 MVP 34개에 포함하지 않으므로 정식 카탈로그도 40개 상한 이하다.
...
제작 progression은 공격력 티어가 아니라 **다룰 수 있는 재료와 만들어낼 수 있는 감각 사건의 확대**다.

| 단계 | 대표 재료·도구 | 플레이 목적 | 범위 원칙 |
|---|---|---|---|
| 1. 즉석 생존 | 깨진 돌, 단단한 가지, 찢은 옷 붕대, 횃불, 식물 섬유 끈 | 채집·해체·점화·긴급 방어·소리 전가 | 시작 복장은 흰 나시와 흰 반바지다. 현대 장비를 찾기 전에도 첫 밤을 넘길 최소 선택을 제공한다. |
| 2. 석기·골각기 사냥 | 돌칼·돌도끼·뼈칼·창·활·투창기·올가미·미끼·함정 | 작은 사냥감을 얻고 랩터의 경로를 제한 | 활은 만능 총기가 아니다. 소형 동물에는 사냥 도구, 랩터에는 부상·경계·유인, 대형 포식자에는 이동 방향을 바꾸는 수단이다. |
| 3. 원시–현대 혼합 | 배터리·철사·렌즈·금속 용기·배관과 수지·기름·가죽·힘줄 | 원격 소음, 집중 조명, 연기·불, 짧은 방어선으로 공간 통제 | 희귀 부품과 무거운 연료를 소비한다. 새 전투 티어가 아니라 기존 감각 시스템을 더 멀리·강하게 작동시키는 후반 선택이다. |

재료는 범용 등급 숫자가 아니라 서로 다른 속성을 가진다. 돌은 날카롭지만 깨지기 쉽고, 뼈는 가볍지만 피 냄새가 남으며, 힘줄은 건조되면 수축하고, 가죽과 수지는 젖음·열에 서로 다르게 반응한다. 플레이어는 월드 관찰과 조합 실험으로 이 속성을 확인한다. 유효한 조합은 노트에 제작법으로 기록하고 실패도 `젖으면 붙지 않음`, `냄새가 너무 강함`, `충돌음 범위가 큼`처럼 다음 판단에 쓸 관찰을 남긴다.

자유 조합 UI가 임의 산출물을 생성하지는 않는다. 조합 가능한 재료 속성과 결과는 작성된 `RecipeData` 범위 안에서 호스트가 판정하며, 노트는 발견 상태와 관찰문만 저장한다. 이 제한은 조합 폭발과 멀티플레이 변조를 동시에 막는다.

활의 정식 확장 화살은 돌촉·뼈촉·불·냄새·소리·줄의 여섯 역할로 제한한다. 투사체마다 새 시뮬레이터를 만들지 않고 기존 피해, `NoiseProfile`, `SmellSource`, `LightSignature`, 로프 상호작용을 조합한다. 불·냄새·소리 화살은 처치보다 흔적을 먼 지점에 배치하는 도구이며, 불은 바람을 타고 번지고 냄새는 다른 포식자도 부르는 역효과를 가진다.

후반 대표 혼합 장비는 **수지 화염 분사 장비**다. 현실 제작 절차는 제공하지 않고 게임 데이터에서 압력 회수 장치, 내열 용기, 가공 연료, 점화부, 호스, 열 차단 보호대의 부품 범주로만 추상화한다. 큰 소리, 희귀하고 무거운 연료, 젖음 점화 실패, 역풍 위험, 산불·연기·원거리 노출을 함께 적용한다. 주 용도는 피해가 아니라 랩터 무리의 길목을 밀어내고 수풀 시야를 열며 대형 포식자를 잠시 멈추는 공간 통제다. MVP에는 넣지 않고 감각·바람·불 확산 관문을 통과한 정식 확장 후보로 둔다.

MVP에서 이 판타지를 증명하는 최소 흐름은 `현대 소모품 의존 → 돌칼·횃불·뼈 긁개 제작 → 스마트폰과 섬유를 결합한 원격 소음 미끼 설치`다. 단궁·특수 화살·혼합 장비 전체를 동시에 만들지 않는다.
...
| 미끼 | **현재 구현:** 날고기 1 | 어디서나, `craft_bait` 1초 데이터 | 산출은 host 권위 복제. 제작 중 냄새는 변경 제안 |
| 뼈 긁개 **현재 구현** | 공룡 뼈 2+힘줄 1 | 어디서나, `craft_bone_scraper` 데이터 | 제작 소음은 `ActionDefinition` 데이터 |
| 돌창 **현재 구현** | 나무 2+돌 1+섬유 1 | `craft_stone_spear` 데이터 | 장착 후 근접 거리·전방 호·스태미나·쿨다운 전투 |
| 활 **현재 구현** | 나무 2+힘줄 1+섬유 1 | `craft_bow` 데이터 | 장착 후 조준·화살 소모·투사체 권위 전투 |
| 작은 가방 **현재 구현** | 가죽 2+섬유 3+힘줄 1 | `craft_small_pack` 데이터 | `back` 슬롯 장비. 용량 확장 시스템 효과는 미구현 |
| 뼈촉 창 **미구현/계획** | 나무 2+공룡 뼈 2+힘줄 1 | 거점 5초, 기존 `stone_spear` 산출 후보 | 180px, 투척 220px |
...
| 원격 소음 미끼 설치 **변경 제안** | 스마트폰 1(회수 가능)+섬유 1 | 지정 유인 자리 3초 | 녹음 재생 260px, 배터리 소비; 설치·회수는 host 권위 stable ID |
| 고속 충전 보정 | 전자 부속 3 | 신호대 5초 | 충전 90→75초 |

자유 제작대 트리와 수십 개 중간재는 만들지 않는다. 현재 `craft_bait`는 `RecipeData`가 조합을 소유하고 `NetCrafting`이 재료·무게·슬롯·현재 권위 인벤토리를 검증한다. **변경 제안:** 제작의 시간·애니메이션·취소·소음은 `ActionDefinition/ActionController`가 담당하되 재료·산출을 중복 저장하지 않으며, 세부 transaction은 `ACTION_SYSTEM_DESIGN.md` §6.6을 따른다.
...
| 건조대 | **현재 구현** | 날고기→말린고기 | 제작 데이터와 거점 상태 저장이 있다. 조리 냄새 확장은 계획 |
| 저장 바구니 | **현재 구현** | 거점 아이템 보관 | 제작 데이터와 호스트 소유권 저장이 있다 |
| 침낭/잠자리 | **현재 구현** | 피로 회복·자동 저장 트리거 | `bedding` 데이터와 거점 상태 저장이 있다. 협동 전원 동의 시간 가속은 계획 |
...
현재 사체 해체 루프는 구현되어 있으며, 사체 단계·절단 소음·피 냄새·산출 `yield_mask`·호스트 권위 지급·재접속/동시성 검증이 있다. 아래 수치의 확장 서술 중 청소형/재왕 사건 연계, 가죽 건조 세부 효과, 일부 장기 청크 델타는 **미구현/계획**이다.
...
- 현재 구현은 3일 압축 세션 30분이다. 7일 기본 세션 70분과 70~90분 목표는 **미구현/계획**이다.
...
2주·1개월은 프롬프트 요구에 따라 확장 방향만 정의했다. 현재 제품 정본은 3일 압축 수직 절편이고 무한 샌드박스가 비목표다. 7일 캠페인의 완주율·반복성이 검증되기 전 장기 생존 모드를 만들지 않는다.
...
현재 `SaveService`는 JSON 파일로 난이도, 플레이어 위치·생존 수치·장비·인벤토리, 시계, 날씨, 목표 결과, 기록, 월드 아이템, 모닥불, 거점 보관/건조/잠자리, 사체, 개체 상태를 저장·복원한다. 아래의 청크 단위 델타 모델, 원자적 tmp/backup 교체, 순차 migration은 장기 월드 확장을 위한 **미구현/계획**이다.
...
| 이벤트 | 완료/보류/쿨다운, 고정 슬롯 결과, 후속 이벤트 | **미구현/계획:** EventDirector |
| 시나리오 | 현재 `LoopObjective` 4결과와 원인 기록. 부품, 진실, 보정, 표현 엔딩 플래그는 계획 | 호스트, 신뢰 전송 |
...
- **미구현/계획:** `save.tmp` 작성→무결성·스키마 확인→기존 최신을 `save.backup`으로 이동→원자적 교체 순서. 현재 구현은 단일 JSON 파일을 직접 쓴다.
...
2. **미구현/계획:** `N → N+1` 순차 migration 함수만 허용한다. 현재 구현은 `FORMAT_VERSION` 불일치 저장을 거부한다.
...
6. 자동 테스트: 현재는 JSON 정수 정규화, 버전 불일치 거부, 주요 저장 섹션 왕복, 사망 복구 규칙을 검증한다. 이전 두 schema fixture 왕복, 강제 종료 후 backup 복원, 핵심 부품 유일성은 **미구현/계획**이다.

저장 파일은 재접속 상태보다 긴 수명의 정본이다. 현재 `SaveService`와 `NetResync`/`NetEquipment`는 일부 스냅샷 표현을 공유하거나 병행하므로, 중복이 더 커질 때만 공통 `PlayerSnapshot` 데이터로 추출한다.
...
├─ SaveService              # 현재 구현: JSON 저장·복원, 원자적 저장·migration은 후속
└─ Main (Node2D)
...
| `scripts/save` | 현재 `SaveService` JSON 저장·복원. 원자적 파일, backup, migration은 계획 | SceneTree 직접 보유 금지, 저장 스키마 검증 유지 |
...
| `SaveService` | 현재 구현 | Scene 교체와 무관한 저장·복원 경계. 원자적 I/O·migration은 후속 책임 |
...
> **변경 공지 — 2026-07-18 정렬:** 아래 MVP 수치표 중 40청크 전체 채움, 7일 70~90분, 부품 3개 필수 탈출, 4인 관련 약속은 새 원칙에 맞춰 재해석한다. 현재 우선순위는 20~30분 3구역 수직 절편, 세계 변수 기반 4결과, 싱글→2인 안정→4인 검증이다.

| 항목 | 현재 검증 프로토타입 | 24주 MVP | 정식 버전 목표 |
|---|---:|---:|---:|
| 맵 | 40청크 골격 + Z01~Z03 열린 수직 절편 | 3구역 절편 밀도 검증 후 5 Zone 확장 판단 | 같은 계곡 5 Zone, 최대 64 Chunk |
...
| 아이템 | 자동 등록 데이터 리소스 다수와 장비/무기/거점 항목 | 34개 상한 | 36개 계획, 40개 이하 |
| 제작법 | 원시 제작 다수, 거점 설비, `craft_small_pack`, host 권위 제작 | 핵심 18개+지정 설비, 고유 총 23개 | 26개 계획, 절대 28개 상한 |
| 이벤트 | 감각 루프·해체 루프 하네스 | **미구현/계획:** 데이터 풀 45개, 세션별 가변 활성 10~15개 | 데이터 풀 유지, 변형 actions 확대 |
| 엔딩 | `STABLE_ESCAPE/FORCED_ESCAPE/REMAIN/FAILED` 4결과 | 4결과 안정화, 표현 최대 6종은 F-C 관문 뒤 | 표현 6종 유지 |
| 시간 | 3일 압축 시계 | 20~30분 3구역 절편 우선, 7일×10분은 확장 목표 | 7일 또는 선택 장기 모드는 검증 후 |
| 플레이 인원 | 싱글/2인 검증 + host+3 수용 하네스 후보 | 싱글+Steam 1~2인 | 4인은 별도 장시간 관문 후 |
| 저장 | `SaveService` JSON 왕복 저장 | 현 저장 안정화, backup/migration은 관문 | 청크·콘텐츠 증가 대응 |

### 22.2 반드시 포함

- 기존 이동·웅크림·소음·냄새·바람·출혈·지혈·모닥불·랩터 감지 루프.
- D01·D03~D05·D07~D12의 화면 등장 10종. 새 자유 이동 AI는 최대 3유형이고 익룡·긴목·티라노·돌등은 사건 경로 연출이다.
- 사체 시드·해체 진행·산출 bit 저장, 공룡 뼈·가죽·힘줄과 뼈 제작의 위험·보상 루프.
- 현재 3일 압축 세션, 세계 변수 기반 4결과 판정, Z01~Z03 수직 절편 완주.
- 5 Zone의 고정 지형 골격, 10개 대표 공간, 시드 자원·사건 변형.
- **미구현/계획:** 청크 스트리밍, 월드 델타, 원자적 저장과 직전 backup.
...
- 돌등·칼등·물갈고리, 재왕 티라노·익룡의 자유 이동 완전 AI, 4인 출시 약속, 장기 생존 모드.
- 이빨칼·힘줄 덫·갈비뼈 건조대 보강의 정식 제작 3종.
- 여러 층, 대형 월드, 차량, 자유 건축, 농사, NPC 외교, 전용 서버, 호스트 이전.
- 공룡 길들이기·탑승·교배, PvP, Workshop, 공식 모드 도구.

**범위 위험:** backup/migration은 W14, 5 Zone·부품 루프는 W18, 이벤트·표현 엔딩 통합은 W20 관문에서 각각 판정한다. 관문을 넘지 못하면 아래 대응 폴백을 즉시 적용하되 기존 랩터 감각 루프, 세계 변수 기반 4결과 판정은 줄이지 않는다.
...
- **F-B 콘텐츠 폴백:** W18에 5 Zone 골격은 유지하되 대표 공간의 추가 변형과 D06·정식 뼈 제작 3종을 연기한다. 완전 AI는 포식 조사형+청소형 2유형으로 줄이고 D05를 비충돌 무리 사건으로 바꾸되, 서로 식별되는 화면 등장 10종·뼈 핵심 제작 4종은 유지한다. 부품 3개 도달성은 7일 확장 관문에서만 요구한다.
...
| 자원 | 자원 수량 증가 | 기준 SpawnTable | 자원 수량 감소 | 4결과 판정의 공정성 |
| 이동 | 일부 길막 감소 | 기준 | 범람·낙석 가중치 증가 | 맵 골격 |
| 시간 | **미구현/계획:** 0.75× 옵션 | 1.0× | **미구현/계획:** 1.25× | 마감 규칙 표시 |
| AI | 포식자 관용도 증가 | 현재 | 포식자 관용도 감소 | 적 체력 뻥튀기 없음 |
| 흔적 | trace feedback duration/intensity 증가 | 현재 | trace feedback duration/intensity 감소 | 호스트 권한·공정성 |

고위험 지역 보상은 단순 자원 수량이 아니다.

- S06: 현재 절편에서는 강변 지름길+공명 기록 후보. 부품 A는 **미구현/계획**.
- S08: 희귀 전자 부속+무리 이동 정보 후보. 부품 B는 **미구현/계획**.
- S10: 신호 보정+표현 엔딩 선택 후보. 부품 C는 **미구현/계획**.
...
| W5~6 **완료/검증 중** | 인벤토리·부상·퀵 제작·3일 시계 | 16칸+무게, 다리 열상 이동 modifier, `craft_bait` host 권위, 3일 HUD/목표 시계 | targeted GUT와 Wave 말 직렬 검증 GREEN | GUT, ENet 5종, replay×3, three_day_slice | 고정 ENet 포트 경합 | 리뷰 문서 PR 1의 port=0/NetTestRig를 후속 기반 부채로 처리 |
...
| W9~10 | 자원·제작·빛 최소 | 아이템 20개 우선, 제작 10개, 소형 사체 1개, 뼈 긁개, 횃불 LightSignature, 원격 소음 미끼 | 해체 소음→피 냄새→랩터 조사와 불·미끼 해법, 원시–현대 혼합 1종 재현 | 총량 불변, 권한 변조, Light/AI, 산출 bit, 설치·회수 중복 | 데이터 필드·UI·복제 | 설명 수치=Resource, B06 예산 통과, 같은 산출 2회 지급 0 |
...
| W13~14 **범위 위험 F-A** | 현재 저장 안정화 + backup/migration 관문 | `SaveService` 회귀, 원자적 저장, 직전 backup, schema fixture, `N→N+1` migration | 저장 20회, 강제 종료 복구, 이전 2개 schema 왕복 | B10, 손상 파일, 사체 산출 중복 | 파일 증가·부분 기록·산출 bit 회귀 | 최신/backup 복구와 migration 전부 통과; 실패 시 F-A 적용 |
...
| W17~18 **범위 위험 F-B** | **미구현/계획:** 5 Zone·7일 부품·생태 루프 | Z04/Z05, 공간 골격 10개, 화면 등장 10종, 완전 AI 최대 3유형, 부품 3개 | 싱글/2인 부품 회수→능선→4결과 위 표현 완주, 종 10개 식별 | 시드 10개 도달성·막힘·AI 상한·save/load | 70분 콘텐츠·종별 에셋 | 막힘 0, 평균 60분 내 능선; 실패 시 F-B로 AI 2유형 전환 |
...
- 4인 출시 약속이나 추가 설정 변경. 2인 청크 분리 부하가 먼저다.
