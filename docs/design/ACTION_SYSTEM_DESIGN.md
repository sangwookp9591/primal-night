...
재료 관찰과 조합 실험으로 제작법을 발견하는 기획도 같은 권위 경계를 사용한다. 클라이언트는 선택한 안정 item ID와 수량만 의도로 보내고, 호스트가 현재 인벤토리·재료 속성·작성된 `RecipeData`를 조회해 성공·실패·소비량·발견할 `recipe_id`를 확정한다. 노트는 자유 텍스트 생성 시스템이 아니라 `known_recipe_ids`와 작성된 성공/실패 관찰 ID를 저장하는 표현 계층이다. 클라이언트가 산출 item ID, 수량, 발견 결과를 보내게 하지 않는다.
...
활·특수 화살·혼합 장비도 별도 전투 프레임워크를 만들지 않는다. 작성된 RecipeData로 아이템을 만들고, 사용 시 기존 `DamageSystem`, `NoiseSystem`, `SmellGrid`, `LightSignature`, 바람/불 상태를 조합한다. 수지 화염 분사 장비는 현실 제작법을 데이터화하지 않으며 정식 확장 전용 공간 통제 행동으로만 취급한다.
